    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["imageFile"]["tmp_name"], $targetFile)) {
            ...
            if ($checkResult && $checkResult->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO blogs (picture, heading, caption) VALUES (?, ?, ?)");
...
$sql = "SELECT heading, MIN(picture) AS picture, MIN(caption) AS caption FROM blogs GROUP BY heading";
...
.col {
    flex: 0 0 33%;
    max-width: 30%;
    padding: 0 15px; /* Adjust for Bootstrap grid padding */
}
...
<body class="sub_page">
    ...
    <section class="about_section layout_padding">
        <div class="container" style="text-align:center">
            <h1>Blogs<a><img src="images/HAND.png" alt="" style="width: 70px;"></a></h1>
        </div>
        <!--image for blog-->
        <div>
        <?php
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<img src='admin/" . $row["picture"] . "' style='width:250px; height:auto;margin-left:25%' />";
        echo "</div>";
